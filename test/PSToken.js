...
contract('PSToken',function(accounts){
    ...
    it('initializes the contract with the correct values',function(){
        return PSToken.deployed().then(function(instance){
            ...
            assert.equal(name,'PSToken','has the correct name');
            return tokenInstance.symbol();
        }).then(function(symbol){
            assert.equal(symbol,'PS','has the correct symbol');
            return tokenInstance.standard();
        }).then(function(standard){
            assert.equal(standard,'PS Token v1.0','has the correct standard');
        });
    });
  ...
  it('handles delegated token transfers',function(){
    ...
    return PSToken.deployed().then(function(instance){
      ...
      return tokenInstance.balanceOf(fromAccount);
        ...
        assert.equal(balance.toNumber(),90,'deducts the number from sending account');
